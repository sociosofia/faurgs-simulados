...
const path='data/source/faurgs_ifsc_questoes_integrais_stage4_linhas.json';
...
for(const [i,q] of qs.entries()){
  ...
  if(!isAnnulled(q)&&/^[A-E]$/.test(gab)&&alts[gab]==null) errors.push(`${id||i+1}: gabarito ${gab} sem alternativa correspondente`);
}
...
if(errors.length){console.error(errors.slice(0,40).join('\n'));process.exit(1);}console.log('Banco FAURGS stage4 com linhas validado.');
